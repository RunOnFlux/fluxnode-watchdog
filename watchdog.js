...
async function Myip() {
  ...
  for (const [index, val] of check_list.entries()) {
    MyIP = await shell.exec(`curl -sk -m 10 https://${val} | tr -dc '[:alnum:].'`, { silent: true })
      .stdout;
...
async function discord_hook(
  ...
  if (typeof web_hook_url !== 'undefined' && web_hook_url !== '0') {
    try {
      ...
      var api_port = shell.exec(`grep -w apiport ${fluxOsConfigPath} | grep -o '[[:digit:]]*'`, {
        silent: true,
      });
      if (api_port == '') {
        var ui_port = 16126;
...
async function Check_Sync(height, time) {
  ...
  var explorer_block_height_02 = await shell.exec(`${exec_comment2}`, {
    silent: true,
  }).stdout;
  var explorer_block_height_03 = await shell.exec(`${exec_comment3}`, {
    silent: true,
  }).stdout;
  ...
  if (height_diff < 12) {
    ...
    if (sync_lock == 0) {
      ...
      if (typeof action == 'undefined' || action == '1') {
        ...
        shell.exec(`sudo systemctl start ${fluxdServiceName}`, {
          silent: true,
        });
        console.log(time + ' => Flux daemon restarting...');
        await discord_hook(
          'Flux daemon restarted!',
          web_hook_url,
          ping,
          'Fix Action',
          '#FFFF00',
...
if (fs.existsSync(configPath)) {
  ...
  if (fs.existsSync(`/usr/local/bin/flux-cli`)) {
    daemon_cli = isArcane ? `flux-cli -conf=${fluxdConfigPath}` : 'flux-cli';
  }
  ...
  if (fs.existsSync(daemonConfigPath)) {
    var tx_hash = shell.exec(
      `grep -w zelnodeoutpoint "${daemonConfigPath}" | sed -e 's/zelnodeoutpoint=//'`,
      {
        silent: true,
      },
    ).stdout;
  ...
  if (!fs.existsSync(daemonConfigPath)) {
    daemonConfigPath = isArcane ? fluxdConfigPath : `${home_dir.trim()}/.flux/flux.conf`;
  }
...
async function auto_update() {
  ...
  const fluxOsStopCmd = isArcane ? 'systemctl stop fluxos.service' : 'pm2 stop flux';

  const fluxOsStartCmd = isArcane ? 'systemctl start fluxos.service' : 'pm2 start flux';
  ...
  var config = require('./config.js');
  var remote_version = shell.exec(
    "curl -sS -m 5 https://raw.githubusercontent.com/RunOnFlux/fluxnode-watchdog/master/package.json | jq -r '.version'",
    { silent: true },
  ).stdout;
  var local_version = shell.exec("jq -r '.version' package.json", {
    silent: true,
  }).stdout;
  ...
  if (remote_version.trim() != '' && local_version.trim() != '') {
    if (remote_version.trim() !== local_version.trim()) {
      ...
      console.log('=================================================================');
      shell.exec(`cd ${watchdogPath} && git checkout . && git fetch && git pull -p`, {
        silent: true,
      }).stdout;
      var local_ver = shell.exec("jq -r '.version' package.json", {
        silent: true,
      }).stdout;
      if (local_ver.trim() == remote_version.trim()) {
        await discord_hook(
          `Fluxnode Watchdog updated!\nVersion: **${remote_version}**`,
          web_hook_url,
          ping,
          'Update',
          '#1F8B4C',
  ...
  if (config.zelflux_update == '1') {
    var zelflux_remote_version = shell.exec(
      "curl -sS -m 5 https://raw.githubusercontent.com/RunOnFlux/flux/master/package.json | jq -r '.version'",
      { silent: true },
    ).stdout;
    var zelflux_local_version = shell.exec(`jq -r '.version' ${fluxOsPkgFile}`, {
      silent: true,
    }).stdout;

    console.log(
      `FluxOS current: ${zelflux_remote_version.trim()} installed: ${zelflux_local_version.trim()}`,
    );
    if (zelflux_remote_version.trim() != '' && zelflux_local_version.trim() != '') {
      if (zelflux_remote_version.trim() !== zelflux_local_version.trim()) {
        ...
        await sleep(5 * 1_000);
        shell.exec(`cd ${fluxOsRootDir} && git checkout . && git fetch && git pull -p`, {
          silent: true,
        }).stdout;
        await sleep(5 * 1_000);
        ...
        await sleep(20);
        var zelflux_lv = shell.exec(`jq -r '.version' ${fluxOsPkgFile}`, {
          silent: true,
        }).stdout;
        if (zelflux_remote_version.trim() == zelflux_lv.trim()) {
          await discord_hook(
            `FluxOS updated!\nVersion: **${zelflux_remote_version}**`,
            web_hook_url,
            ping,
            'Update',
            '#1F8B4C',
  ...
  if (config.zelcash_update == '1') {
    var zelcash_remote_version = shell.exec(
      "curl -s -m 5 https://apt.runonflux.io/pool/main/f/flux/ | grep -o '[0-9].[0-9].[0-9]' | head -n1",
      { silent: true },
    ).stdout;
    var zelcash_local_version = shell.exec(`dpkg -l flux | grep -w flux | awk '{print $3}'`, {
      silent: true,
    }).stdout;
    console.log(
      `Flux daemon current: ${zelcash_remote_version.trim()} installed: ${zelcash_local_version.trim()}`,
    );
    if (zelcash_remote_version.trim() != '' && zelcash_local_version.trim() != '') {
      if (zelcash_remote_version.trim() !== zelcash_local_version.trim()) {
        ...
        var update_info = shell.exec(
          "ps -C apt,apt-get,dpkg >/dev/null && echo 'installing software' || echo 'all clear'",
          { silent: true },
        ).stdout;
        ...
        var zelcash_dpkg_version_before = shell.exec(
          `dpkg -l flux | grep -w flux | awk '{print $3}'`,
          {
            silent: true,
          },
        ).stdout;
        ...
        var zelcash_dpkg_version_after = shell.exec(
          `dpkg -l flux | grep -w flux | awk '{print $3}'`,
          {
            silent: true,
          },
        ...
        if (
          zelcash_dpkg_version_before !== zelcash_dpkg_version_after &&
          zelcash_dpkg_version_after != ''
        ) {
          await discord_hook(
            `Fluxnode daemon updated!\nVersion: **${zelcash_dpkg_version_after}**`,
            web_hook_url,
            ping,
            'Update',
            '#1F8B4C',
  ...
  if (config.zelbench_update == '1') {
    var zelbench_remote_version = shell.exec(
      "curl -s -m 5 https://apt.runonflux.io/pool/main/f/fluxbench/ | grep -o '[0-9].[0-9].[0-9]' | head -n1",
      { silent: true },
    ).stdout;
    var zelbench_local_version = shell.exec(
      "dpkg -l fluxbench | grep -w fluxbench | awk '{print $3}'",
      {
        silent: true,
      },
    ).stdout;
    ...
    console.log(
      `Fluxbench current: ${zelbench_remote_version.trim()} installed: ${zelbench_local_version.trim()}`,
    );
    ...
    if (zelbench_remote_version.trim() != '' && zelbench_local_version.trim() != '') {
      if (zelbench_remote_version.trim() !== zelbench_local_version.trim()) {
        ...
        var zelbench_dpkg_version_before = shell.exec(
          `dpkg -l fluxbench | grep -w fluxbench | awk '{print $3}'`,
          {
            silent: true,
          },
        ...
        var zelbench_dpkg_version_after = shell.exec(
          `dpkg -l fluxbench | grep -w fluxbench | awk '{print $3}'`,
          {
            silent: true,
          },
        ...
        if (
          zelbench_dpkg_version_before !== zelbench_dpkg_version_after &&
          zelbench_dpkg_version_after != ''
        ) {
          await discord_hook(
            `Fluxnode benchmark updated!\nVersion: **${zelbench_dpkg_version_after}**`,
            web_hook_url,
            ping,
            'Update',
            '#1F8B4C',
...
async function flux_check() {
  const fluxbenchServiceName = isArcane ? 'fluxbenchd.service' : 'zelcash.service';

  const fluxOsRestartCmd = isArcane ? 'sudo systemctl restart fluxos.service' : 'pm2 restart flux';
  ...
  const service_inactive = shell.exec(
    `systemctl list-units --full -all | grep ${fluxdServiceName} | grep -o inactive`,
    { silent: true },
  ).stdout;
  ...
  if (zelbench_counter > 2 || zelcashd_counter > 2 || zelbench_daemon_counter > 2) {
    try {
      var zelcash_height = shell.exec(`${daemon_cli} getblockcount`, {
        silent: true,
      }).stdout;
      var zelbench_getstatus_info = JSON.parse(
        shell.exec(`${bench_cli} getstatus`, { silent: true }).stdout,
      );
    ...
    if (
      typeof zelcash_height !== 'undefined' &&
      isNumber(zelcash_height) &&
      zelbench_benchmark_status != 'toaster' &&
      zelbench_benchmark_status != 'failed' &&
      typeof zelbench_benchmark_status !== 'undefined'
    ) {
      zelcashd_counter = 0;
      zelbench_counter = 0;
      ...
      if (zelcashd_counter == 3 || zelbench_counter == 3) {
        if (sleep_msg == '0') {
          sleep_msg = 1;
          await discord_hook(
            'Watchdog in sleep mode..\nManual operation needed!',
            web_hook_url,
            ping,
            'Alert',
            '#EA1414',
            'Info',
            'watchdog_manual1.png',
            label,
          );
  ...
  try {
    var zelbench_getbenchmarks_info = JSON.parse(
      shell.exec(`${bench_cli} getbenchmarks`, { silent: true }).stdout,
    );
  ...
  try {
    var zelcash_getzelnodestatus_info = JSON.parse(
      shell.exec(`${daemon_cli} getzelnodestatus`, { silent: true }).stdout,
    );
  ...
  if (
    typeof zelbench_status == 'undefined' &&
    typeof zelcash_height !== 'undefined' &&
    isNumber(zelcash_height)
  ) {
    ...
    if (typeof action == 'undefined' || action == '1') {
      shell.exec(`sudo systemctl stop ${fluxbenchServiceName}`, {
        silent: true,
      });
      await sleep(2 * 1_000);
      if (!isArcane) shell.exec('sudo fuser -k 16125/tcp', { silent: true });
      shell.exec(`sudo systemctl start ${fluxbenchServiceName}`, {
        silent: true,
      });
      console.log(data_time_utc + ' => Flux benchmark restarting...');
      await discord_hook(
        'Flux benchmark restarted!',
        web_hook_url,
        ping,
        'Fix Action',
        '#FFFF00',
    ...
    zelbench_daemon_counter != 0 &&
    ['CUMULUS', 'NIMBUS', 'STRATUS'].includes(zelbench_benchmark_status)
  ) {
    await discord_hook(
      'Flux benchmark fixed!',
      web_hook_url,
      ping,
      'Fix Info',
      '#1F8B4C',
  ...
  if (zelback_status == '' || typeof zelback_status == 'undefined') {
    ...
    if (zelback_status == 'disconnected') {
      ...
      if (lock_zelback != '1' && disc_count == 2) {
        ...
        var flux_api_port = shell.exec(
          `grep -w apiport ${fluxOsConfigPath} | grep -o '[[:digit:]]*'`,
          {
            silent: true,
          },
        );
        var port_api = Number(flux_api_port.trim());
        var error_output = shell.exec(
          `curl -sSL -m 10 http://localhost:${port_api}/id/loginphrase`,
          {
            silent: true,
          },
        ).stdout;
      ...
      if (typeof action == 'undefined' || action == '1') {
        if (disc_count == 2) {
          ...
          shell.exec(`sudo systemctl restart ${fluxdServiceName}`, {
            silent: true,
          });
          await sleep(2 * 1_000);
  ...
  if (zelbench_benchmark_status == '' || typeof zelbench_benchmark_status == 'undefined') {
    ...
    if (zelbench_benchmark_status == 'toaster' || zelbench_benchmark_status == 'failed') {
      ...
      await discord_hook(
        'Benchmark ' + zelbench_benchmark_status + ' \n**Reason:**\n' + zelbench_error,
        web_hook_url,
        ping,
        'Alert',
        '#EA1414',
      ...
      var field_type = 'Error: ';
      var msg_text =
        'Benchmark ' + zelbench_benchmark_status + ' \u{274C} \n<b>Reason:</b>\n' + zelbench_error;
      await send_telegram_msg(emoji_title, info_type, field_type, msg_text, label);
    } else {
      console.log('Benchmark status = ' + zelbench_benchmark_status);
    }
  ...
  if (zelbench_time == 'null' || zelbench_time == '' || typeof zelbench_time == 'undefined') {
    ...
    const next_benchmark_time = moment(timestamp, 'DD/MM/YYYY HH:mm:ss')
  ...
  if (
    zelcash_last_paid_height == 'null' ||
    zelcash_last_paid_height == '' ||
    typeof zelcash_last_paid_height == 'undefined'
  ) {
  } else {
  ...
  if (zelbench_benchmark_status == 'toaster' || zelbench_benchmark_status == 'failed') {
    ++zelbench_counter;
    var error_line = shell.exec(
      `egrep -a --color 'Failed' ${fluxbenchLogPath} | tail -1 | sed 's/[0-9]\{4\}-[0-9]\{2\}-[0-9]\{2\}.[0-9]\{2\}.[0-9]\{2\}.[0-9]\{2\}.//'`,
      { silent: true },
    );
  ...
  if (
    config.tier_eps_min != '' &&
    config.tier_eps_min != '0' &&
    zelbench_eps != '' &&
    zelbench_eps < config.tier_eps_min
  ) {
    ++tire_lock;
    if (tire_lock < 4) {
      error(
        'Benchmark problem detected! CPU eps under minimum limit for ' +
          tire_name +
          '(' +
          eps_limit +
          '), current eps: ' +
          zelbench_eps.toFixed(2),
      );
      console.log('Benchmark problem detected!');
      console.log(
        'CPU eps under minimum limit for ' +
          tire_name +
          '(' +
          eps_limit +
          '), current eps: ' +

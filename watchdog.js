  ...
  if (typeof web_hook_url !== "undefined" && web_hook_url !== "0") {
    if (typeof ping == "undefined" || ping == "0") {
      ...
      if (typeof label == "undefined") {
        const msg = new webhook.MessageBuilder().setTitle(`:loudspeaker: **FluxNode ${title}**`).addField("URL:", `http://${node_ip}:${ui_port}`).addField(`${field_name}:`, node_msg).setColor(`${color}`).setThumbnail(`https://fluxnodeservice.com/images/${thumbnail_png}`);
        await Hook.send(msg);
      } else {
      ...
      if (typeof label == "undefined") {
        const msg = new webhook.MessageBuilder().setTitle(`:loudspeaker: **FluxNode ${title}**`).addField("URL:", `http://${node_ip}:${ui_port}`).addField(`${field_name}:`, node_msg).setColor(`${color}`).setThumbnail(`https://fluxnodeservice.com/images/${thumbnail_png}`).setText(`Ping: <@${ping}>`);
        await Hook.send(msg);
      } else {
        const msg = new webhook.MessageBuilder()
          .setTitle(`:loudspeaker: **FluxNode ${title}**`)
...
async function Check_Sync(height, time) {
  ...
  if (height_diff < 12) {
    ...
    if (sync_lock == 0) {
      ...
      if (typeof action == "undefined" || action == "1") {
        shell.exec("sudo systemctl stop zelcash && sleep 2", { silent: true });
...
if (fs.existsSync(path)) {
  ...
  try {
    fs.writeFileSync(path, dataToWrite, { flag: "w" });
  ...
  if (config.zelflux_update == "1") {
    ...
    if (zelflux_remote_version.trim() != "" && zelflux_local_version.trim() != "") {
      if (zelflux_remote_version.trim() !== zelflux_local_version.trim()) {
        ...
        console.log("=================================================================");
        shell.exec("pm2 stop flux && sleep 5", { silent: true }).stdout;
        shell.exec("cd /home/$USER/zelflux && git checkout . && git fetch && git pull -p && sleep 5", { silent: true }).stdout;
        shell.exec("pm2 start flux && sleep 20", { silent: true }).stdout;
  ...
  if (config.zelcash_update == "1") {
    ...
    if (zelcash_remote_version.trim() != "" && zelcash_local_version.trim() != "") {
      if (zelcash_remote_version.trim() !== zelcash_local_version.trim()) {
        ...
        var zelcash_dpkg_version_after = shell.exec(`dpkg -l flux | grep -w flux | awk '{print $3}' && sleep 2`, { silent: true }).stdout;
        shell.exec("sudo systemctl start zelcash", { silent: true });
  ...
  if (config.zelbench_update == "1") {
    ...
    if (zelbench_remote_version.trim() != "" && zelbench_local_version.trim() != "") {
      if (zelbench_remote_version.trim() !== zelbench_local_version.trim()) {
        ...
        shell.exec("sudo fuser -k 16125/tcp", { silent: true });
        shell.exec("sudo apt-get update", { silent: true });
        shell.exec("sudo apt-get install fluxbench -y && sleep 2", { silent: true });
        shell.exec("sudo systemctl start zelcash", { silent: true });
...
async function flux_check() {
  ...
  try {
    let fluxbenchStatus = shell.exec(`${bench_cli} getstatus`, { silent: true }).stdout;
  ...
  try {
    let fluxbenchInfo = shell.exec(`${bench_cli} getbenchmarks`, { silent: true }).stdout;
  ...
  try {
    let fluxDaemonInfo = shell.exec(`${daemon_cli} getinfo`, { silent: true }).stdout;
  ...
  try {
    let fluxnodeStatus = shell.exec(shell.exec(`${daemon_cli} getzelnodestatus`, { silent: true }).stdout);
    ...
    if (zelback_status == "disconnected") {
      ...
      if (typeof action == "undefined" || action == "1") {
        if (disc_count == 2) {
          ...
          shell.exec("sudo systemctl restart zelcash && sleep 2", { silent: true });
